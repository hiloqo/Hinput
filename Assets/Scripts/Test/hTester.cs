...
public class hTester : MonoBehaviour {
	...
	public bool stickSimpleDirections;
	...
	void Update () {
		if (individualGamepads) {
			for (int i=0; i<hInput.maxGamepads; i++) TestWholeGamepad (hInput.gamepad[i]);
		}

		if (anyGamepad) TestWholeGamepad (hInput.anyGamepad);
	}
	...
	private void TestWholeGamepad (hGamepad gamepad) {
		TestButtons (gamepad);
		TestSticks (gamepad);
	}
	...
	private void TestSticks (hGamepad gamepad) {
		if (currentStick == null || currentStick.inDeadZone) {
		...
		if (currentStick != null) {
			if (stickPosition) Debug.Log (currentStick.fullName+" position : "+currentStick.position);
			if (stickPositionRaw) Debug.Log (currentStick.fullName+" position : "+currentStick.positionRaw);
	...
	private void TestButtons (hGamepad gamepad) {
		if (currentButton == null || !currentButton.pressed) {
			foreach (hAbstractInput button in AllGamepadButtons(gamepad)) {
				if (button.pressed) {
					currentButton = button;
					if (pressedAndReleased) Debug.Log(button.fullName+" is pressed !");
				}
			}
		}

		if (currentButton != null) {
			if (pressedAndReleased && !currentButton.pressed) Debug.Log (currentButton.fullName+" is released !");
			...
			if (lastPressWasDouble) Debug.Log (currentButton.fullName+"'s last press was "+(currentButton.lastPressWasDouble?"":"not ")+"a double press");
			if (longPress) {
			...
			if (lastPressWasLong) Debug.Log (currentButton.fullName+"'s last press was "+(currentButton.lastPressWasLong?"":"not ")+"a long press");
	...
	private List<hAbstractInput> AllGamepadButtons (hGamepad gamepad) {
		...
		if (stickSimpleDirections) buttons.AddRange (new List<hAbstractInput>() {
